import {  EyeOff, LogIn ,Mail , Eye ,Lock  } from 'lucide-react'
...
useEffect(()=>{
  ...
  if(token){
    (async ()=>{
      try{
        const {data}=await axios.get(`${API_USER}/me`,{
...
const handleSubmit=async (e)=>{
  e.preventDefault()
  if(!rememberMe){
    toast.error("Your must enable 'Remember Me ' to login.")
    return
  }
  setLoading(true)

  try{
    const {data}=await axios.post(`${API_USER}/login`,formData)
    if(!data.token){
    ...
    <span className="flex items-center justify-center gap-1.5">
      ...
      Login
            ...
            <p className='text-center text-sm text-gray-600 mt-6'>
              Don't have account?{' '}
              <button type='button' className='text-purple-500' onClick={handleSwitchMode}>Sign Up</button>
            </p>
        </div>
  )
}
